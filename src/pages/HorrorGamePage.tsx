    ...
    <div className="min-h-screen bg-[#050308] text-slate-100">
      ...
      {phase === 'menu' && (
        <div className="mx-auto flex max-w-md flex-col items-center gap-6 px-4 py-16 text-center">
          ...
          <button
            onClick={createRoom}
            disabled={connecting || !supabaseConfigured}
            className="btn-outline w-full border-slate-600 text-slate-200"
          >
          ...
          <div className="flex w-full gap-2">
            ...
            <button
              onClick={joinRoom}
              disabled={connecting || !supabaseConfigured}
              className="btn-outline shrink-0 border-slate-600 text-slate-200"
            >
              Unirse
            </button>
          </div>
          {!supabaseConfigured && (
            <p className="text-xs text-amber-500">
              Modo en equipo desactivado: falta configurar VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY (ver README).
            </p>
          )}
